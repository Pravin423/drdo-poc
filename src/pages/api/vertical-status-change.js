export default async function handler(req, res) {
  ...
  if (!id) {
    return res.status(400).json({ status: false, message: "Vertical ID is required" });
  }
  ...
  try {
    const response = await fetch(
      `https://goadrda.runtime-solutions.net/admin/api/vertical-status-change/${id}?status=${status}`,
      {
        method: "GET",
        headers: {
    ...
    console.error("Vertical Status Change Proxy Error:", error);
