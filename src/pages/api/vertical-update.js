export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch("https://goadrda.runtime-solutions.net/admin/api/vertical/update", fetchOptions);
    const data = await response.json();

    return res.status(response.status).json(data);
  } catch (error) {
    console.error("Proxy fetch error (vertical-update):", error);
