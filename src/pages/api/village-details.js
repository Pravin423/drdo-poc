export default async function handler(req, res) {
  ...
  if (!id) {
    return res.status(400).json({ status: false, message: "Village ID is required" });
  }

  try {
    const response = await fetch(
      `https://goadrda.runtime-solutions.net/admin/api/villages/details/${id}`,
      {
        method: "GET",
        headers: {
    ...
    try {
      ...
      return res.status(response.status).json({ status: response.ok, message: `Failed to fetch village details (status: ${response.status})` });
    }
    
    return res.status(response.status).json(data);
  } catch (error) {
    console.error("Proxy fetch error (village details):", error);
