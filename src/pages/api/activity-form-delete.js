// src/pages/api/activity-form-delete.js
export default async function handler(req, res) {
  ...
  if (!id) {
    return res.status(400).json({ status: false, message: "Activity Form ID is required" });
  }

  // Basic validation that ID is a string of digits/alphanumerics
  if (!/^[a-zA-Z0-9_-]+$/.test(id.toString())) {
    return res.status(400).json({ status: false, message: "Invalid Activity Form ID format" });
  }

  try {
    const response = await fetch(
      `https://goadrda.runtime-solutions.net/admin/api/activity-forms/${id}`,
      {
        method: "DELETE",
        headers: {
    ...
    return res.status(response.status).json({
      status: 0,
      message: errorData.message || errorData.error || `Failed to delete activity form (HTTP ${response.status})`,
      details: errorData
    });
