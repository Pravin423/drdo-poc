export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `https://goadrda.runtime-solutions.net/admin/api/villages/update`,
      {
    ...
    try {
      ...
      return res.status(response.status).json({ status: response.ok, message: `Failed to update village (status: ${response.status})` });
    }
    
    return res.status(response.status).json(data);
  } catch (error) {
    console.error("Proxy update error (village):", error);
